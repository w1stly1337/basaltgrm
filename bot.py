...
bot = Bot(token='7288998294:AAE2Gjh2CkXDSfOBzr5LZ9HuKEgGrGrCHl0') # токен давным давно ревокнут можешь не пытаться чето сделать хех ххе
dp = Dispatcher()
...
async def add_free_number(uid: str | int) -> str:
    numbers = await get_numbers()
    phone = f'1{random.randint(111_111_1111, 999_999_9999)}'
...
async def start(message: types.Message) -> None:
    ...
    if any(keyboard.inline_keyboard):
        keyboard.inline_keyboard.append([types.InlineKeyboardButton(text='✅ Я подписался', url='http://t.me/Basaltgrambot?start=1')])
